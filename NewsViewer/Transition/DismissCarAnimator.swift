...
final class DismissCardAnimator: NSObject, UIViewControllerAnimatedTransitioning {
    ...
    struct Params {
        ...
        let fromCell: NewsCollectionViewCell
    }
    ...
    func animateTransition(using transitionContext: UIViewControllerContextTransitioning) {
        ...
        // Fix weird top inset
        let topTemporaryFix = screens.cardDetail.newsCardContentView.topAnchor.constraint(equalTo: cardDetailView.topAnchor)
        topTemporaryFix.isActive = GlobalConstants.isEnabledWeirdTopInsetsFix

        container.layoutIfNeeded()

        // Force card filling bottom
        let stretchCardToFillBottom = screens.cardDetail.newsCardContentView.bottomAnchor.constraint(equalTo: cardDetailView.bottomAnchor)
